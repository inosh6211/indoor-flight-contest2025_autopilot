...
void pulseISR_de(){
  ...
  if(Period_de>0){
    Duty_de=(double)PW_de/(double)Period_de;
...
float thrust(float T,float Vc,float AoA,float B,float theta,float psi,float phi,float Twb[3][3]){//control law of motor
  float Kp=1;
  float Kd=1;
  float M=0.25;
  float g=9.81;
