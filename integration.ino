...
volatile uint32_t endTime_S = 0;
volatile uint32_t startTime_p_S = 0;

//reciever
int pin_num_SVe_R = 1;
int pin_num_SVa_R = 3;
int pin_num_MT_R = 4;
float Duty_de = 0.0, Duty_da = 0.0, Duty_MT = 0.0;
volatile uint32_t startTime_de = 0, startTime_da = 0, startTime_MT = 0;
volatile uint32_t endTime_de = 0, endTime_da = 0, endTime_MT = 0;
volatile uint32_t startTime_p_de = 0, startTime_p_da = 0, startTime_p_MT = 0;
...
void loop() {
  ...
  noInterrupts();
  uint32_t ST_de = startTime_de;
  uint32_t ET_de = endTime_de;
  uint32_t STp_de = startTime_p_de;
  uint32_t ST_da = startTime_da;
  uint32_t ET_da = endTime_da;
  uint32_t STp_da = startTime_p_da;
  uint32_t ST_MT = startTime_MT;
  uint32_t ET_MT = endTime_MT;
  uint32_t STp_MT = startTime_p_MT;
  uint32_t ST_S = startTime_S;
  uint32_t ET_S = endTime_S;
  uint32_t STp_S = startTime_p_S;
  interrupts();
  uint32_t PW_S = ET_S - ST_S;
  uint32_t Period_S = ST_S - STp_S;
  if(Period_S>0){
...
//////////////////////////////
      ...
      //receive
      uint32_t PW_de = ET_de - ST_de;
      uint32_t Period_de = ST_de - STp_de;
      uint32_t PW_da = ET_da - ST_da;
      uint32_t Period_da = ST_da - STp_da;
      uint32_t PW_MT = ET_MT - ST_MT;
      uint32_t Period_MT = ST_MT - STp_MT;
    if(Period_de>0){
